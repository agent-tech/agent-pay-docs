...
metadata: {"category":"payment","blockchains":["solana","base","ethereum","polygon","hyperevm","arbitrum","bsc","monad","skale-base","megaeth"],"protocol":"x402","sdk":"cross402","mode":"public"}
---
...
When `payment_requirements.extra.assetTransferMethod === "permit2"` the payer chain requires the Permit2 flow (designated for BSC, Monad, MegaETH; the exact set is whatever the backend flags). This flow requires **two signatures**:
...
async function completeX402PaymentFlow(
  ...
  if (pr.extra?.assetTransferMethod === 'permit2') {
    // Permit2 + EIP-2612 path (designated for BSC, Monad, MegaETH; the backend flags the exact set)
