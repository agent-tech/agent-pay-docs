...
- **EIP-3009 `TransferWithAuthorization`** — Circle USDC on Base / Ethereum / Polygon / Arbitrum; USDT0 on Arbitrum / Monad / HyperEVM / MegaETH. A single signed authorization is sufficient; the facilitator relays it on-chain.
- **Permit2 + EIP-2612** — chains with non-standard USDC (BSC, Monad, MegaETH); and **Polygon USDT / USDT0** (same contract `0xc2132D...`, implements EIP-2612 on a salted EIP-712 domain — not EIP-3009). The payload combines a Permit2 `PermitWitnessTransferFrom` signature with a gasless EIP-2612 `Permit` that grants Permit2 the allowance. Cross402 sponsors the on-chain submission. For Polygon USDT/USDT0, `payment_requirements.extra.domainType == "salted"` signals the non-standard domain layout.
