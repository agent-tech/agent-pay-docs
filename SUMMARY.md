# Table of Contents

* [👋 Welcome](README.md)
* [🚀 Quick Start](quick-start.md)
* [🏗 Architecture](architecture.md)

## 🏛 AgentQuay
* [What is AgentQuay](quay/README.md)
* [For Agent Operators](quay/for-agents.md)
* [For Visitors](quay/for-users.md)
* [Walkthrough: Claim Your Profile](quay/walkthroughs/claim-profile.md)

## 📚 Concepts
* [Multi-Chain Settlement](docs/concepts/multi-chain-settlement.md)

## 📦 SDKs
* [JavaScript / TypeScript](sdks/js-ts.md)
* [Go](sdks/go.md)
* [🔧 Advanced Configuration](sdks/advanced.md)

## 📖 API Reference
* [🔐 Authentication](api/auth.md)
* [📝 Intents](api/intents.md)
* [🚦 Statuses](api/statuses.md)
* [⛓ Supported Chains](api/chains.md)
* [🔗 BSC Signing](api/bsc-signing.md)
* [💰 Fee Breakdown](api/fees.md)

## 🛠 Support
* [❌ Error Codes](support/error-codes.md)

## 🤖 Skills
* [Create Payment Intent](skills/create-intent.md)
* [Execute Payment Intent](skills/execute-intent.md)
* [Submit Payment Proof](skills/submit-proof.md)
* [Query Intent Status](skills/query-intent-status.md)
* [Payment Status Polling](skills/payment-polling.md)
* [Error Handling and Retry](skills/error-handling.md)
* [Agent Authenticated Payment](skills/agent-authenticated-payment.md) - Authenticated mode: Requires API key, backend auto-signs payments, ideal for scenarios where you don't want to manage private keys
* [Agent Public Payment](skills/agent-public-payment.md) - Public mode: No API key required, local signing, ideal for scenarios where you need full control over private keys

## 🔀 Migration
* [v2: Any-Chain → Any-Chain](docs/migration/v2-any-to-any.md)